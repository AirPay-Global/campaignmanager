...
export default function Dashboard() {
  ...
  const { data:msgsData, isLoading:msgsLoading } = useQuery<MessagesResponse>({ queryKey:['messages-outbound'], queryFn:()=>api.get('/messages/outbound?limit=1000').then(r=>r.data) });
  const { data:campsData, isLoading:campsLoading } = useQuery<CampaignsResponse>({ queryKey:['campaigns'], queryFn:()=>api.get('/campaigns').then(r=>r.data) });
  const { data:contsData } = useQuery<ContactsResponse>({ queryKey:['contacts'], queryFn:()=>api.get('/contacts').then(r=>r.data) });
    ...
    <div style={{ padding:'32px', minHeight:'100vh', background:'#06060f', position:'relative' }}>
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(190px,1fr))', gap:16, marginBottom:28 }}>
          ...
          <StatCard title="Active Campaigns"    value={activeCampaigns}    icon={Megaphone}     colorIdx={1} delay={60}  />
          <StatCard title="Messages Today"      value={sentToday}          icon={MessageSquare} colorIdx={2} delay={120} />
          <StatCard title="Delivery Rate"       value={`${deliveryRate}%`} icon={TrendingUp}    colorIdx={3} delay={180} />
        </div>
      ...
      {msgsLoading && <span style={{display:'none'}} aria-live="polite">Loading…</span>}
    </div>
  );
}
